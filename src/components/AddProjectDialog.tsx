...
const projectTypes = ["Internship", "Contract", "Part-Time", "Full-Time", "Freelance"];
...
export function AddProjectDialog({ open, onOpenChange, projectToEdit }: AddProjectDialogProps) {
  ...
  useEffect(() => {
    if (open) {
      if (projectToEdit) {
        ...
        setTitle(""); setType(""); setLocation(""); setDescription("");
        setSelectedSkills([]); setBudget(""); setDeadline(""); setDuration(""); setCurrentSkill("");
      }
    }
  }, [open, projectToEdit]);
  ...
  const handleSkillKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter') { e.preventDefault(); handleSkillAdd(currentSkill); }
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.info("Project save endpoint not yet connected to backend.");
        ...
        <div className="space-y-6 py-4">
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-2 col-span-2">
              ...
              <Input id="title" value={title} onChange={(e) => setTitle(e.target.value)}
                className="border-border" placeholder="e.g. Website Redesign" />
            </div>
            ...
            <div className="space-y-2">
              ...
              <Select value={location} onValueChange={setLocation}>
                <SelectTrigger className="border-border"><SelectValue placeholder="Select location" /></SelectTrigger>
                <SelectContent>
          ...
          <div className="space-y-2">
            ...
            <Textarea id="description" value={description} onChange={(e) => setDescription(e.target.value)}
              className="min-h-[100px] border-border resize-none" placeholder="Describe the project details..." />
          </div>
            ...
            <div className="flex flex-wrap gap-2 mb-2 p-2 border rounded-md min-h-[42px] bg-background">
              {selectedSkills.map((skill) => (
                <Badge key={skill} variant="secondary" className="px-2 py-1 flex items-center gap-1">
                  {skill}
                  <X className="h-3 w-3 cursor-pointer hover:text-destructive" onClick={() => handleSkillRemove(skill)} />
                </Badge>
              ))}
            ...
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap gap-2">
                {suggestedSkills.map((skill) => (
                  <Badge key={skill} variant="outline" className="cursor-pointer hover:bg-secondary transition-colors"
          ...
          <div className="grid grid-cols-3 gap-4">
            <div className="space-y-2">
              ...
              <Input id="budget" type="number" value={budget} onChange={(e) => setBudget(e.target.value)}
                className="border-border" placeholder="0.00" />
            </div>
            <div className="space-y-2">
              ...
              <Input id="duration" type="number" value={duration} onChange={(e) => setDuration(e.target.value)}
                className="border-border" placeholder="e.g. 30" />
            </div>
            <div className="space-y-2">
          ...
          <div className="flex justify-end pt-4">
            <Button onClick={handleSubmit} disabled={isLoading} className="bg-primary hover:bg-primary/90 px-8">
